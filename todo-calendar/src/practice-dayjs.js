...
export const runPracticeDayjs = () => {
  ...
  const now = dayjs("2024-04-01 11:44:12");
  console.log("===== Practice Dayjs =====");
  console.log("1. set minute - hh", dayjs(now).set("minute", 5).format("YYYY.MM.DD hh:mm:ss a A"));
  console.log("2. set minute - HH", dayjs(now).set("minute", 5).format("YYYY.MM.DD HH:mm:ss a A"));
  console.log("3. set hour - hh", dayjs(now).set("hour", 10).format("YYYY.MM.DD HH:mm:ss a A"));
  console.log("4. get your", dayjs(now).get("year"));
